...
export default function NavBar({ isDark, setIsDark, itemCount, inventoryData, isLoggedIn }){

    const [cartImage, setCartImage] = useState(null);

    const inventory = isLoggedIn ? inventoryData : "";
    
    let cartLength = itemCount;
            ...
            <nav className='flex items-center justify-between p-1'>
                ...
                <ul className='flex'>
                    ...
                    <li className="grid grid-cols-2 ">
                        {isLoggedIn ? ( <NavLink className={({ isActive }) => ["m-2", "p-3", "pt-4", "content-evenly", "truncate","rounded-xl","border-textColor","bg-none", "scale-90", "text-lg/6", "text-textColor", "hover:scale-100", "font-mono", isActive ? "border" : ""].join(" ")} to='/account'> Account </NavLink> )
                       : ( <NavLink className={({ isActive }) => ["m-2", "p-3", "pt-4" ,"content-evenly","rounded-xl","border-textColor","bg-none", "scale-90", "text-lg/6", "truncate", "text-textColor", "hover:scale-100", "font-mono", isActive ? "border" : ""].join(" ")} to='/login'> SignIn </NavLink> )}
                       <div>
                        <NavLink className={({ isActive }) => ["absolute", "border-textColor", "rounded-xl", "bg-none", "mt-2", "scale-90", "px-4", "py-3", "hover:scale-100", isActive ? "border" : ""].join(" ")} to='/cart'> <img src={cartImage} alt="cartImg" className='size-12'/>  <p className="right-4 text-xs -top-0 top-3 fixed font-bold text-textColor"> { itemCount > 0 ?  itemCount : "" } </p> </NavLink>
                       </div>
                    </li>
                </ul>
            </nav>
